...
class DataReaderRemote:
    ...
    def read_data(self, chunk_size: int):
        ...
        """
        data_type = str(self.config['reading']['reading_mode']).lower()
    ...
    def _read_csv(self, chunk_size: Optional[int]) -> Generator[pl.DataFrame, None, None]:
        ...
        Lê um arquivo CSV.
        """
        host = self.config['reading']['host']
        user = self.config['reading']['user']
        password = self.config['reading']['password']
        path = self.config['reading']['caminho']
        filename = self.config['reading']['nome_arquivo']
    ...
    def _read_database(self, chunk_size: Optional[int]) -> Generator[pl.DataFrame, None, None]:
        ...
        user = self.config['reading']['user']
        password = self.config['reading']['password']
        database = self.config['reading']['database']
        type_database = self.config['reading']['type_database']
        query = self.config['reading']['query']
...
class DataReaderFile:
    ...
    def _read_csv(self) -> Generator[pl.DataFrame, None, None]:
        ...
        return pl.read_csv(full_path, truncate_ragged_lines=True)

    def _read_json(self) -> Generator[pl.DataFrame, None, None]:
        """
        Lê um arquivo JSON em chunks.
        """
        path = self.config['reading']['caminho']
        filename = self.config['reading']['nome_arquivo']
    ...
    def _read_parquet(self) -> Generator[pl.DataFrame, None, None]:
        """
        Lê um arquivo Parquet em blocos (chunks).
        """
        path = self.config['reading']['caminho']
        filename = self.config['reading']['nome_arquivo']
